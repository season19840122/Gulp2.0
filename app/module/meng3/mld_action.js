var ACTIONFUN = function(initObj) {
	...
	var actionObj = {
		getMelee : {
			name:"获取时间段详情",
			url: function(gameId) {
				return initObj.options.hostUrl+"melee/getPanelMeleeInfo?gameId="+gameId;
			}
		},
		listRecords : {
			name:"获取达成奖励名单",
	...
	this.eventElement = function() {
		$(function(){
			...
			//顺豆支付绑定hover
			$('.content_bottom .award_box li').hover(function(){
			...
			//跳转积分商城
			$('.status_box a').on('click',function() {
				goVip();
			})
	...
	var setInfo = function(obj) {
		...
		if((state == 1 && hasUserApply && hasUserGameFinished) || (state == 1 && !hasUserApply)) {
			...
			if(obj.applySwBean == 0) {
				isShunHover = false;
				$('.award_box li').eq(0).find('h3').html("￥"+obj.applyMoney);
				$('.award_box li').eq(1).find('h3').html("￥"+obj.applyMoney*5);
				$('.award_box li').eq(2).find('h3').html("￥"+obj.applyMoney*10);
				$('.award_box li').eq(3).find('h3').html("￥"+obj.applyMoney*20);
			} else {
				isShunHover = true;
				$('.award_box li').eq(0).find('h2').html(obj.applySwBean);
				$('.award_box li').eq(1).find('h2').html(obj.applySwBean*5);
				$('.award_box li').eq(2).find('h2').html(obj.applySwBean*10);
				$('.award_box li').eq(3).find('h2').html(obj.applySwBean*20);
			}
			
			now_gameId = GameID.M3GUO;
			now_period = obj.period;
			var accountList =  ClientAPI.getLoginGameList();
			
			accountList.map(function(v) {
				if(v.gameId == GameID.M3GUO) now_account = v.subAccount;
			})
