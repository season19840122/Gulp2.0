var optionObj = function(options_){
	...
	this.alertMsg = function(msg) {
		$('#alert_box .modal-inline-center').html(msg);
		$('#alert_box').modal();
	}
	...
	var fresh = function(){
		chestObj.ajax_({
			...
			success: function(json) {
				data = {
			  	num: 24 - (json.data.pieces.length + json.data.chests.length),
			  	pieces: json.data.pieces,
			    chests: json.data.chests
			  };
			  chestVM.addPieceClass(chestVM.chests);
			}
		});
	};
	fresh();
