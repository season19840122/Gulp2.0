var ACTIONFUN = function(initObj) {
	...
	this.eventElement = function() {
		$(function(){
			...
			$('#alert_join').on('click','.btn_sure', function() {
				now_applyCnt = $('#alert_join input[name=limitEachCnt]').val();
	...
	var setInfo = function(obj) {
		...
		var rule3 = "3、玩家需在"+obj.calculateTime+"之前完成"+returnGameList.join("")+"的游戏，"+obj.gameTypeRemark+"，"+(obj.allowTeam?"允许组队；":"不允许组队；")+"</span>";
		...
		if((state == 1 && hasUserApply && hasUserGameFinished) || (state == 1 && !hasUserApply)) {
			...
			for(var i = 1;i<=obj.limitEachCnt;i++) {
				optionArr.push("<li>"+i+"</li>");
			}
			if(obj.limitEachCnt > 3) $('#alert_join input[name=limitEachCnt]').siblings('ul').addClass('isScroll');
			$('#alert_join input[name=limitEachCnt]').val(1);
			$('#alert_join input[name=limitEachCnt]').siblings('ul').html(optionArr.join(""));
			...
			//份数下拉框内容点击
			$("#limitEachCnt li").off('click');
			$("#limitEachCnt li").click(function(e) {
				var txt = $(this).html();
				var applySwBean = $('#alert_join input[name=applySwBean]').val();
				var applyMoney = $('#alert_join input[name=applyMoney]').val();
				if(applySwBean == 0) {
					$('#alert_join .all').html("￥"+initObj.accMul(applyMoney,txt));
				} else {
					$('#alert_join .all').html(applySwBean*txt+"顺豆/￥"+initObj.accMul(applyMoney,txt));
				}
				$(this).parents(".drop-select ul").siblings("input").val(txt).removeClass("open");
				$(".drop-select ul").hide();
			});
